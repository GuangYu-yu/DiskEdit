...
/// 主头几何状态。Stale = 结构可识别但 backup header 停在旧末端（设备扩容后未搬移）；
/// 该状态不算合法 GPT，info 只报告，由写入路径 relocate 修复
...
/// 保护 MBR 的 SizeInLBA 与当前容器的关系（UEFI 2.10 §5.2.3：SizeInLBA = 磁盘扇区数 − 1，
/// 超出 32 位表示范围时写 0xFFFFFFFF）
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum PmbrSize {
    /// 与当前容器一致（含大盘合法的 0xFFFFFFFF 饱和写法）
    Normal,
    /// 小于当前容器：扩容后的 stale PMBR，写入路径按当前容器重写即可
    Stale,
    /// 大于当前容器：损坏/不一致，只报告，拒绝自动修复
    Inconsistent,
}
...
/// 备份头早于末端 = Stale（设备扩容后未搬移，由写入路径经 ensure_geometry relocate 修复）；
/// 越过末端 = 拒绝
fn validate_geometry(header: &RawHeader, file_last_lba: u64) -> io::Result<GptState> {
    if header.primary_lba != 1 {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "GPT primary_lba != 1 — invalid GPT"));
    }
    if header.first_usable_lba > header.last_usable_lba {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "first_usable_lba > last_usable_lba — invalid GPT"));
    }
    if header.last_usable_lba > file_last_lba {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "last_usable_lba beyond container end — invalid GPT"));
    }
    if header.backup_lba > file_last_lba {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "backup GPT header beyond container end — truncated image"));
    }
    Ok(if header.backup_lba == file_last_lba {
        GptState::Valid
    } else {
        GptState::Stale { backup_lba: header.backup_lba, file_last_lba }
    })
}

/// 解析主头 + 条目数组（全部自研，写入路径用；保证主头有效）。
/// `pmbr` 由调用方（load_gpt）判定后传入——PMBR 是独立结构，不随扇区候选变化。
/// 返回 None = 该扇区大小下 LBA1 无 GPT。
fn parse_primary(src: &FileSource, ss: u64, pmbr: PmbrSize) -> io::Result<Option<RawGpt>> {
    ...
    let es = header.size_of_partition_entry;
    // 16 MiB 为自定防御上限（非 UEFI 要求）：几何异常即拒绝，不按表字段做巨型分配
    if n == 0 || !valid_entry_size(es) || n * es as u64 > 16 * 1024 * 1024 {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "implausible GPT entry geometry"));
    }
    let es = es as u64;
    // 损坏表的 lba/size 字段不受信任，乘加全部 checked，防溢出回绕
    let array_off = header
        .partition_entry_lba
        .checked_mul(ss)
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "GPT entry array offset overflow"))?;
    let array_len = n * es;
    if array_off.checked_add(array_len).ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "GPT entry array range overflow"))? > src.size {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "GPT entry array out of range"));
    }
    let mut raw = vec![0u8; array_len as usize];
    src.read_at(array_off, &mut raw)?;
    let stored = rd_u32(&sec, 88);
    if crc32(&raw) != stored {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "GPT entry array CRC mismatch"));
    }
    // 几何自洽性校验（validate_geometry），失败即拒绝；下游可对其结果做 LBA 算术
    let state = validate_geometry(&header, src.size / ss - 1)?;
    let mut entries = Vec::with_capacity(n as usize);
    for i in 0..n as usize {
        let e = &raw[i * es as usize..i * es as usize + 128];
        entries.push(parse_entry(e));
    }
    Ok(Some(RawGpt { ss, header, entries, state, pmbr }))
}
...
/// - 备头：MyLBA=last_lba, Alt=1, 数组=last_lba-span
fn canonical_headers(g: &RawGpt, last_lba: u64) -> (RawHeader, RawHeader) {
    let span = array_span_sectors(g.header.number_of_partition_entries, g.header.size_of_partition_entry, g.ss);
    ...
    let backup = RawHeader {
        ...
        last_usable_lba: g.header.last_usable_lba,
        disk_guid: g.header.disk_guid,
        partition_entry_lba: last_lba - span,
        ..primary.clone()
    };
    (primary, backup)
}
...
pub fn commit_gpt(src: &mut FileSource, g: &RawGpt, last_lba: u64) -> io::Result<()> {
    let (primary, backup) = canonical_headers(g, last_lba);
...
/// SizeInLBA = 扇区数 − 1，超出 32 位表示范围才用 0xFFFFFFFF（UEFI 2.10 §5.2.3）。
...
pub fn resize_mdos_entry(src: &mut FileSource, part: u32, new_size_lba: u32) -> io::Result<()> {
    if !(1..=4).contains(&part) {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, format!("invalid MBR partition number {part} (1..4)")));
    }
    let ss = src.sector_size as usize;
    if src.size < ss as u64 {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "image too small for MBR"));
    }
    let mut lba0 = vec![0u8; ss];
    src.read_at(0, &mut lba0)?;
    if u16::from_le_bytes([lba0[510], lba0[511]]) != MBR_SIGNATURE {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "no MBR signature on target"));
    }
    let off = 446 + (part as usize - 1) * 16;
    let rec = &mut lba0[off..off + 16];
    if rec[4] == 0 {
        return Err(io::Error::new(io::ErrorKind::NotFound, format!("MBR partition {part} is empty")));
    }
    if matches!(rec[4], 0x05 | 0x0F | 0x85) {
        return Err(io::Error::new(io::ErrorKind::PermissionDenied, "extended partition container cannot be resized"));
    }
...
/// SizeInLBA 与当前容器的关系：期望值按设备逻辑扇区计（UEFI 口径），也接受部分工具按 512B
/// 计的写法；扇区数超出 32 位表示范围时按规范饱和写 0xFFFFFFFF（此时两种口径期望值相同）。
/// 小于当前容器 = stale PMBR（可重写修复）；大于 = 损坏/不一致（拒绝自动修复）
fn pmbr_size_state(src: &FileSource) -> io::Result<PmbrSize> {
    ...
    // e ≤ e512（扇区大小 ≥ 512），故 size < e 即小于两种口径
    Ok(if size < e { PmbrSize::Stale } else { PmbrSize::Inconsistent })
}
...
pub fn load_gpt(src: &FileSource) -> io::Result<Option<RawGpt>> {
    ...
    for ss in [src.sector_size, 512, 4096] {
        if let Some(g) = parse_primary(src, ss, pmbr)? {
            return Ok(Some(g));
        }
    }
    Ok(None)
...
pub fn add_entry_at(
    ...
    let mut g = crate::movepart::ensure_geometry(src)?.ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "no GPT"))?;
    if start < g.header.first_usable_lba || end > g.header.last_usable_lba || start > end {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!("range {start}..{end} outside usable {}..{}", g.header.first_usable_lba, g.header.last_usable_lba),
        ));
    }
    for e in &g.entries {
        if e.ending_lba == 0 {
        ...
        if !(end < e.starting_lba || start > e.ending_lba) {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "range overlaps an existing partition"));
        }
    }
    let Some(slot) = g.entries.iter().position(|e| e.ending_lba == 0) else {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "partition table full"));
    };
...
/// 超长部分由 `From<&str>` 静默截断
pub fn rename_entry(src: &mut FileSource, part: u32, name: &str) -> io::Result<()> {
    if part == 0 {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, format!("invalid partition number {part} (1-based)")));
    }
    let mut g = crate::movepart::ensure_geometry(src)?
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "no GPT"))?;
...
pub fn set_gpt_flag(src: &mut FileSource, part: u32, flag: &str, on: bool) -> io::Result<()> {
    if part == 0 {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, format!("invalid partition number {part} (1-based)")));
    }
    ...
    let bit: u64 = match flag {
        ...
        "hidden" => 1 << 1,
        "required" => 1 << 0,
        other => return Err(io::Error::new(io::ErrorKind::InvalidInput, format!("unknown gpt flag {other} (esp/legacy/hidden/required)"))),
    };
    let mut g = crate::movepart::ensure_geometry(src)?
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "no GPT"))?;
    let e = g.entries.get_mut((part - 1) as usize)
...
fn is_mdos_label(src: &FileSource) -> io::Result<bool> {
    Ok(table_label(src)? == "msdos")
}

/// 标签判定："gpt" / "msdos" / "none"
pub fn table_label(src: &FileSource) -> io::Result<&'static str> {
...
pub fn add_mdos_entry(src: &mut FileSource, start: u64, end: u64, os_type: u8) -> io::Result<u32> {
    let ss = src.sector_size;
    if !is_mdos_label(src)? {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "not an msdos-labelled image"));
    }
    let total = src.size / ss;
    if total == 0 {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "image has zero sectors"));
    }
    if start < 1 || end >= total || start > end {
        return Err(io::Error::new(io::ErrorKind::InvalidData, format!("range {start}..{end} outside disk (1..{})", total - 1)));
    }
    let mut lba0 = vec![0u8; ss as usize];
    src.read_at(0, &mut lba0)?;
    ...
    let Some(i) = slot else {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "all 4 primary slots used (extended/logical unsupported)"));
    };
...
pub fn del_mdos_entry(src: &mut FileSource, part: u32) -> io::Result<()> {
    if !(1..=4).contains(&part) {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "msdos supports primary slots 1..=4 only"));
    }
    let ss = src.sector_size;
    let mut lba0 = vec![0u8; ss as usize];
    src.read_at(0, &mut lba0)?;
    ...
    if rec[4] == 0 {
        return Err(io::Error::new(io::ErrorKind::NotFound, format!("partition {part} is already empty")));
    }
    rec.fill(0);
    src.write_at(0, &lba0)?;
    src.sync_all()
...
pub fn set_mdos_boot(src: &mut FileSource, part: u32, on: bool) -> io::Result<()> {
    if !(1..=4).contains(&part) {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "msdos supports primary slots 1..=4 only"));
    }
    let ss = src.sector_size;
...
pub fn set_mdos_hidden(src: &mut FileSource, part: u32, on: bool) -> io::Result<()> {
    if !(1..=4).contains(&part) {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "msdos supports primary slots 1..=4 only"));
    }
    let ss = src.sector_size;
    ...
    if next == 0 {
        return Err(io::Error::new(
            ...
            format!("type 0x{cur:02X} has no hidden counterpart (supported: fat12/16/32 incl. lba, ntfs)"),
        ));
    }
    lba0[off + 4] = next;
    src.write_at(0, &lba0)?;
...
pub fn del_entry(src: &mut FileSource, part: u32) -> io::Result<()> {
    if part == 0 {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, format!("invalid partition number {part} (1-based)")));
    }
    let mut g = crate::movepart::ensure_geometry(src)?
...
mod tests {
    ...
    fn protective_mbr_shape_and_size() {
        ...
        assert!(pmbr_shape_valid(&src).unwrap());
        assert_eq!(pmbr_size_state(&src).unwrap(), PmbrSize::Stale);
        // 等于容器 = Normal；大于容器 = 不一致（拒绝自动修复）；0 亦视为 stale
