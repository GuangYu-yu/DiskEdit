...
**占用边界**：块设备写入靠整盘 `O_EXCL` 独占打开——任一分区被挂载或作 swap 时，内核以 EBUSY 拒绝打开；另有占用闸在写盘前复核（块设备上是纵深防御）。镜像文件没有这层内核语义，占用闸只对块设备分区节点生效；镜像内分区被 `losetup` 映射并挂载时本工具不检测该占用，对同一镜像的并行使用在支持边界之外。
